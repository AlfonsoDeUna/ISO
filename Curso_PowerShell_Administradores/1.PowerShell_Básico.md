...
Aquí tienes la continuación del documento Markdown, enfocada en la lógica condicional múltiple y el manejo de colecciones de datos, utilizando el ejemplo de la calculadora que propusiste.
...
Un **Array** es como una caja compartimentada donde puedes guardar varios valores bajo un mismo nombre. En lugar de tener `$nombre1`, `$nombre2`, `$nombre3`, guardamos todo en una sola lista.
...
```powershell
$frutas = "Manzana", "Pera", "Plátano", "Cereza"

foreach ($fruta in $frutas) {
    Write-Host "Hoy voy a comer una pieza de: $fruta"
}
```

### Explicación sencilla:

1. **`$frutas`**: Es el Array (la cesta completa).
2. **`$fruta`**: Es una variable temporal que representa el elemento actual que estamos procesando.
...
¿Te gustaría que prepare una sección sobre cómo **crear funciones** para que tus alumnos puedan reutilizar el código de la calculadora?
